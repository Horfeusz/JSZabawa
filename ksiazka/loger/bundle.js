(function (global, factory) {
	...
	(factory((global.logger = {}),global.os));
...
var reverseKeywords = {};
for (var key in index$7) {
	if (index$7.hasOwnProperty(key)) {
		reverseKeywords[index$7[key]] = key;
	}
}
...
convert.rgb.keyword = function (rgb) {
	...
	for (var keyword in index$7) {
		if (index$7.hasOwnProperty(keyword)) {
			var value = index$7[keyword];
...
const wrapAnsi16 = (fn, offset) => function () {
	const code = fn.apply(index$5, arguments);
...
const wrapAnsi16m = (fn, offset) => function () {
	const rgb = fn.apply(index$5, arguments);
...
function assembleStyles() {
	...
	for (const key of Object.keys(index$5)) {
		if (typeof index$5[key] !== 'object') {
			continue;
		}

		const suite = index$5[key];
...
var index$11 = function (flag, argv) {
...
let supportLevel = (() => {
	if (index$11('no-color') ||
		index$11('no-colors') ||
		index$11('color=false')) {
		return 0;
	}

	if (index$11('color=16m') ||
		index$11('color=full') ||
		index$11('color=truecolor')) {
		return 3;
	}

	if (index$11('color=256')) {
		return 2;
	}
...
var index$9 = process && support(supportLevel);
...
function applyOptions(obj, options) {
	...
	obj.level = options.level === undefined ? index$9.level : options.level;
...
for (const key of Object.keys(index$3)) {
	index$3[key].closeRe = new RegExp(index$1(index$3[key].close), 'g');

	styles[key] = {
		get() {
			const codes = index$3[key];
			return build.call(this, this._styles ? this._styles.concat(codes) : [codes], key);
		}
	};
}

index$3.color.closeRe = new RegExp(index$1(index$3.color.close), 'g');
for (const model of Object.keys(index$3.color.ansi)) {
	...
	styles[model] = {
		get() {
			...
			return function () {
				const open = index$3.color[levelMapping[level]][model].apply(null, arguments);
				const codes = {
					open,
					close: index$3.color.close,
					closeRe: index$3.color.closeRe
				};
...
index$3.bgColor.closeRe = new RegExp(index$1(index$3.bgColor.close), 'g');
for (const model of Object.keys(index$3.bgColor.ansi)) {
	if (skipModels.has(model)) {
		continue;
	}
	...
	styles[bgModel] = {
		get() {
			const level = this.level;
			return function () {
				const open = index$3.bgColor[levelMapping[level]][model].apply(null, arguments);
				const codes = {
					open,
					close: index$3.bgColor.close,
					closeRe: index$3.bgColor.closeRe
				};
				return build.call(this, this._styles ? this._styles.concat(codes) : [codes], model);
			};
		}
...
function applyStyle() {
	...
	const originalDim = index$3.dim.open;
...
var supportsColor_1 = index$9;

index.supportsColor = supportsColor_1;

const Logger = {
    success: function (msg) {
        console.log(index.green.bold('OK: '), msg);
    },
    error: function (msg) {
        console.error(index.red.bold('Error: '), msg);
    },
    warning: function (msg) {
        console.warn(index.yellow.bold('Warning: '), msg);
    },
    info: function (msg) {
        console.log(index.blue.bold('Info: '), msg);
    }
};
